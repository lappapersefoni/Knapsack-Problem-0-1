...
std::vector <item> brute_force_solver(knapsack_problem& ks) {
    ...
    for (int i = 0;i < total;i++)
    {
        ...
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        lastSolutionTime = duration.count();
        if (lastSolutionTime > 10000) {//ks.time_limit){
            std::cout << "Time out: t>" << ks.time_limit << "\n";
            return result;
        }
    }
            ...
            for (int k3 = 0;k3 < 4;k3++) {
                for (int k4 = 1;k4 <= 5;k4++) {
                    ...
                    if ((selection == 0) || (selection == 5)) {
                        fout.close();
                        std::string filenameWithArguments = "dynamic_ortools.exe " + filename_string + " " + csv_path;
                    ...
                    if ((selection == 0) || (selection == 6)) {
                        fout.close();
                        std::string filenameWithArguments = "integer_ortools.exe " + filename_string + " " + csv_path;
                        std::cout << "Filename: " << filenameWithArguments << "\n";
                        int retCode = system(filenameWithArguments.c_str());
                        fout.open(csv_path, std::ios::app);
                    }
...
int main() {
    ...
    std::cout << "Please select an algorithm to solve the knapscack problem \n";
    std::cout << "Please insert your selection from 1 to 7 \n";
    std::cout << "1.Greedy Approach algorithm\n";
    std::cout << "2.Brute Force algorithm\n";
    std::cout << "3.Branch and Bound algorithm\n";
    std::cout << "4.Dynamic algorithm\n";
    std::cout << "5.Dynamic ortools aglorithm\n";
    std::cout << "6.Integer ortools aglorithm\n";
